...
describe('GET /api/github', () => {
  ...
  describe('Unrestricted Cache Bypass & Abuse Mitigation (Issue #1978)', () => {
    ...
    it('Scenario 1: serves cached data and checks SWR background refresh', async () => {
      ...
      expect(getFullDashboardData).toHaveBeenCalledWith('torvalds', expect.objectContaining({ bypassCache: false }));
    ...
    it('Scenario 2: allows a single refresh request when limits are respected', async () => {
      ...
      expect(response.status).toBe(200);
      expect(getFullDashboardData).toHaveBeenCalledWith('torvalds', expect.objectContaining({ bypassCache: true }));
    ...
    it('Scenario 3: serves cached response for repeated refresh requests within cooldown', async () => {
      ...
      expect(getFullDashboardData).toHaveBeenLastCalledWith('torvalds', expect.objectContaining({ bypassCache: true }));
      ...
      expect(getFullDashboardData).toHaveBeenLastCalledWith('torvalds', expect.objectContaining({ bypassCache: false }));
